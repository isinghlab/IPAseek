...
createSE <- function(project.dir, count.files.dir = NULL, data.table.path, design.colmns = NULL){

	# project.dir = project.dir
	# count.files.dir = count.files.dir
	# data.table.path = datainfo.location_uniq
	# design.colmns = c("RACE", "GENDER", "ETHNICITY", "VITAL_STATUS", "AGE_AT_INDEX", "ISS_STAGE", "TUMOR_DESCRIPTOR")

	df.design <- read.delim(data.table.path, sep = "\t", header  = T, stringsAsFactors= T)
	...
	counts.files <- list.files(path = count.files.dir, pattern = "*_gene_counts.rds", full.names = T)
